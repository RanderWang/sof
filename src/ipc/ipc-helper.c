			       ...
			       struct comp_buffer *buffer)
{
	...
	ret = pipeline_connect(comp, buffer, PPL_CONN_DIR_COMP_TO_BUFFER);
			       ...
			       struct comp_dev *comp)
{
	...
	tr_dbg(&ipc_tr, "ipc: comp sink %d, source %d  -> connect", comp->ipc_config.id,
	       buffer->id);
	...
	ret = pipeline_connect(comp, buffer, PPL_CONN_DIR_BUFFER_TO_COMP);
